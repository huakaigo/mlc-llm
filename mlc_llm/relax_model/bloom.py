...
class BloomGelu(nn.Module):
    r"""Bloom Gelu is same with the relax.op.gelu_tanh
    ...
    ----
        ...
        transform.bloom.model.BloomGelu
            .. math::
...
class BloomGeluX(nn.Module):
    r"""BloomGeluV1 cannot get right result
...
class BloomMLP(nn.Module):
    ...
    def forward(self, x: relax.Expr):
        ...
        ## TODO 不清楚 castdown float32 to float16会有什么不良影响, 需要再研究下llama代码如何混用float32
...
def apply_alibi_pos_emb(q: relax.Expr, k: relax.Expr, alibi: relax.Expr, alpha, beta = 1.0):
    """deprecated. merge with attn_weight calculation

    caclulation
    ---
    out = beta * alibi + alpha * matmul(q, k)

    where,
    
        q.shape = [*, seq_len, head_dim]

        k.shape = [*, head_dim, kv_seq_len]

        alpha = 1.0 / math.sqrt(head_dim), is a constant value for one model

        and beta is always 1.0, so skip beta * alibi

    finally, we get: out = alibi + alpha * (q @ k)
    """
    pass
...
def build_alibi_tensor(batch_size, seq_length, num_heads, dtype="float32"):
    ...
    # attention_mask = nn.emit(relax.op.ones((batch_size, seq_length), dtype=dtype))
    # cumsum_mask = nn.emit((relax.op.cumsum(attention_mask, axis=-1) - relax.const(1.0, dtype=dtype)) * attention_mask)
    print("=====")
    print(batch_size)
    print(batch_size.value)
    # type(batch_size) = tvm.tir.IntImm, 通过batch_size.value获取标量
...
class BloomAttention(nn.Module):
    ...
    def __init__(self, config: BloomConfig):
        ...
        # output [bsz, seq_length, num_heads * 3 * head_dim]
    ...
    def _split_heads(self, fused_qkv: relax.Expr) -> Tuple[relax.Expr, relax.Expr, relax.Expr]:
        ...
        reshape_qkv = nn.emit(reshape(fused_qkv, (bsz, seq_length, 3 * self.num_heads, self.head_dim)))
        # print(f"fused_qkv.shape = {reshape_qkv.struct_info.shape}")
        sub_tensors = nn.emit(relax.op.split(reshape_qkv, indices_or_sections = 3, axis=2))
        q_tensor = nn.emit(sub_tensors[0])
        k_tensor = nn.emit(sub_tensors[1])
        v_tensor = nn.emit(sub_tensors[2])
        # print(f"q_tensor.shape = {q_tensor.struct_info.shape}")
    ...
    def forward(
        ...
        # print(f"hidden states dtype: {hidden_states.struct_info.dtype}")
        ...
        # [bsz, seq_len, num_heads, head_dim] -> [bsz, num_heads, kv_seq_len, head_dim]
        ...
        # print(f"alibi.shape= {alibi.struct_info.shape}")
        # print(f"query_states.shape= {query_states.struct_info.shape}")
        # print(f"key_states.shape= {key_states.struct_info.shape}")
        attn_weights = nn.emit(
            # matmul(query, key): [bsz, num_heads, q_len, head_dim] @ [bsz, num_heads, head_dim, kv_seq_len]
            # result.shape = [bsz, num_heads, q_len, kv_seq_len]
...
class BloomBlock(nn.Module):
    def __init__(self, config: BloomConfig):
        ...
        self.self_attention = BloomAttention(config = config)
...
class BloomModel(nn.Module):
    def __init__(self, config: BloomConfig, sep_embed: bool = False):
        ### TODO config中没有pad_token_id, 在toekenizer.json中找到'<pad>' id 为 3
        self.padding_idx = 3
    ...
    def forward(
        ...
        # retrieve input_ids
        print(inputs_embeds)
        print(inputs_embeds.struct_info)
        print(inputs_embeds.struct_info.shape)
